...
#### 不用 resample
* 只需要给 SDL 输送数据的时候按照 SDL 需要的格式输送即可，重采样太耗时了，而且容易出问题。
* SDL 需要的格式是 `AUDIO_S16SYS`，即 16 bit native 字节序的格式。

##### 假如播放的文件音频流格式是 s16p 的，那么我们就这么转换：
* 首先看一下 s16p 的音频数据是怎么存储的
...
* 根据 `AVFrame` 里 `extended_data` 的注释：
...
* 注意，Planar 格式的音频数据在 `AVFrame` 的 `data[0~n]` 和 `extended_data[0~n]` 都是一样的
* 但当声道数超过了 `data[]` 的表示范围，则 `extended_data[]` 能保存所有的声道信息
* 毕竟 `uint8_t **extended_data`  而 `uint8_t *data[AV_NUM_DATA_POINTERS]`， data[] 的长度是有限的。

* 注意 `AVFrame` 里 `linesize` 字段的含义，每个声道有同样的大小，都等于`linesize[0]`，`linesize[1]`以后的没意义：

		For audio, only linesize[0] may be set. For planar audio, each channel plane must be the same size.

* 注意 `AVFrame` 里 `nb_samples` 字段的含义，是**每个声道的**采样个数：
...
* 知道了以上这些，就可以改造一下 tutorial03.cpp 了，让它可以支持多种音频格式，而不是只支持 s16 这种市面上几乎已经绝种的音频格式。

		把原来代码里的 memcpy(audio_buf, frame.data[0], data_size); 这一行，替换成下面这一段
		...
		{	
			for (int i = 0; i < ch_nu; i++)
			{
				...
				memcpy(audio_buf, frame.extended_data[i]+offset, bytes_per_sample);
				audio_buf += bytes_per_sample;
			}
...
##### 假如播放的文件音频流格式是 fltp 的，那么我们就这么转换：
* 参考：[http://my.oschina.net/u/589963/blog/168301](http://my.oschina.net/u/589963/blog/168301)
* 这里面有一处注释应该是写错了：
