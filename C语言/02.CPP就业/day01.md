...
#### 拷贝构造函数
* 复习结构体赋值(00基础\09.)。
* 当把**栈上**的一个对象赋值给另一个对象的时候，会调用类的拷贝构造函数。
	* 注意，堆上的对象你只能得到其指针，因为 new 返回的是指针。所以堆上的对象互相赋值，只是让指针指过去而已，不涉及拷贝构造函数。
	
			Person *p1 = new Person(25,'xiaozhang');
			Person *p2 = p1; // 只是让p2指向p1同样地方，不会调用 Person 的拷贝构造函数，因为根本没重新分配内存。
	* 如果没有显示的定义拷贝构造函数，则编译器自动把所有成员变量对于复制，类似于结构体的互相赋值。
* 显示定义拷贝构造函数的格式是：定义一个参数是本类的const引用的构造函数，形如：`Person(const Person &p){...}`
* 为了避免浅拷贝的风险，如果类里面有指针类型的成员变量，我们要显示的定义拷贝构造函数。在拷贝构造函数里自己分配内存，然后把已有对象的值复制进去：

		name = new char[20];
		strcpy(name, p.name);

* 如果函数的参数是对象，那么函数入栈时会调用拷贝构造函数
* 如果类特别的大，那么把类对象作为函数参数，每次调用函数时都在栈空间新做一个临时类对象，并且调拷贝构造函数进行拷贝，这样效率特别低。
* 所以一般不把类对象直接做函数参数，可以传指针或者传引用。
* 为了不在函数内部改变这个对象，可以把参数设计为 const 引用。

#### const 对象 / const 成员函数
* const 对象的意思是，这个对象的所有成员变量都是 const的，是不能改变的。
	
		const Person p = {10,"xiaozhang"};
		p.age = 20; //错误， const 对象的成员变量的值不能被改变
* const 的对象，不能调用成员函数，编译器认为任何成员函数都可能改变成员变量的值。
	
		const Person p = {10,"xiaozhang"};
		p.get_age(); //错误， const 对象的成员函数不能被调用
* 除非某个成员函数声明为了 const，告诉编译器，我这个函数是不改变任何成员变量的值的，这样的成员函数是可以被调用的。

		class Person{
			public:
			char* get_name() const {...} // 这种函数可以被 const 对象调用
			}
* 声明为 const 的语法如下，把 const 写在后面。
